...
import { CalendarCheck, Sparkles, Copy, Repeat, Store, Trash2, Settings, Clock } from 'lucide-react';
...
export default function Home() {
  ...
  // Initialize selected store (with fallback to first available store)
  useEffect(() => {
    if (selectedStoreId) return;
    if (user?.store_ids?.[0]) {
      setSelectedStoreId(user.store_ids[0]);
    } else if (stores.length > 0 && !selectedStoreId) {
      // Fallback: if user has no store_ids yet (new user), use first available store
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).filter(e => e.all_stores || !e.store_id || (selectedStoreId && e.store_id === selectedStoreId));
    },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-slate-100 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-3 sm:py-5">
          ...
          {/* 提出期限バッジ - スマホでは別行に表示 */}
          {(activeDeadline || (simpleDeadline && !activeDeadline)) && (
            <div className="mt-2 sm:mt-2.5">
              <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-purple-500 to-pink-600 text-white px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg shadow text-xs sm:text-sm">
                <Clock className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                <span className="font-semibold">
                  {activeDeadline
                    ? `期限: ${format(parseISO(activeDeadline.deadline_date), 'M月d日', { locale: ja })}まで`
      ...
      {isAdminOrManager && deadlineDialogOpen && (
