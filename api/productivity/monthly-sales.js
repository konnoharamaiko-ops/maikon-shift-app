/**
 * Vercel Serverless Function: Monthly Sales from TempoVisor
 * テンポバイザーから指定年月の月間売上データを取得
 * 
 * エンドポイント: GET /api/productivity/monthly-sales?year=2026&month=3&store_name=田辺店
 * レスポンス: { success: true, year_month: "2026-03", store_name: "田辺店", total_sales: 1234567 }
 */
...
async function fetchMonthlySalesFromTempoVisor(username, password, year, month, storeName) {
  ...
  // テーブルから取得できなかった場合、ページ全体のテキストから合計を探す
  if (totalSales === 0) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!username || !password) {
      return res.status(500).json({ error: 'TempoVisor credentials not configured' });
    }
